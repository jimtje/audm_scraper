...
class Episode(object):
    ...
    def __init__(self, filename, relative_dir, root_url):
        ...
        self.tags = MP4(self.filename).tags
        try:
            self.mp4 = MP4(self.filename)
        except Exception:
            self.mp4 = None
    ...
    def as_html(self):
        ...
        return template.render(
            ...
            date=formatdate(self.date), description=self.description, author=self.author, subtitle=self.subtitle,
                summary=self.summary,
            image_url=self.image,
        )
    ...
    def title(self):
        ...
        text = self.tags.get('\xa9nam')[0]
        return text

    @property
    def author(self):
        text = self.tags.get('\xa9ART')[0]
        return text

    @property
    def description(self):
        title = self.tags.get('\xa9nam')[0]
        author = self.tags.get('\xa9ART')[0]
        pub = self.tags.get('\xa9alb')[0]
        readby = self.tags.get('\xa9wrt')[0]
        text = self.tags.get('desc')[0]
        return title + " by " + author + "\n From " + pub + ", read by " + readby + "\n\n" + text

    @property
    def subtitle(self):
        title = self.tags.get('\xa9alb')[0]
        title += ": " + self.tags.get('\xa9nam')[0]
        return title
